...
<head>
    ...
    <title>Daftar - Shellemerch</title>
            ...
            <div class="col-md-6 col-lg-5 col-xl-4">
                <div class="card login-card p-4">
                    <div class="text-center mb-4 mt-2">
                        ...
                        <h5 class="mt-3 fw-bold text-dark">Buat Akun</h5>
                        <p class="text-muted small mb-0">Dengan akun, semua pesanan pin custom Anda tercatat dan bisa dipantau.</p>
                    </div>

                    <form action="{{ route('register.post') }}" method="POST" novalidate>
                        @csrf
                        <div class="mb-3">
                            <label for="name" class="form-label text-muted small fw-bold ms-2">Nama</label>
                            <input id="name" type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" autocomplete="name" required autofocus>
                        ...
                        <div class="mb-3">
                            ...
                            <input id="email" type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" autocomplete="email" required>
                            @error('email')<div class="invalid-feedback ms-2">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-3">
                            <label for="phone" class="form-label text-muted small fw-bold ms-2">Nomor HP / WhatsApp</label>
                            <input id="phone" type="tel" name="phone" class="form-control @error('phone') is-invalid @enderror" value="{{ old('phone') }}" placeholder="08xxxxxxxxxx" autocomplete="tel" required>
                            @error('phone')<div class="invalid-feedback ms-2">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-3">
                            <label for="password" class="form-label text-muted small fw-bold ms-2">Password</label>
                        ...
                        <div class="mb-4">
                            <label for="password_confirmation" class="form-label text-muted small fw-bold ms-2">Ulangi password</label>
                    ...
                    <div class="text-center mt-4" style="font-size: 14px;">
                        ...
                        <a href="{{ route('login') }}" class="fw-semibold text-decoration-none" style="color: #2A6CA2;">Masuk</a>
                    </div>
                    <div class="text-center mt-3 mb-2">
                        <a href="{{ url('/') }}" class="text-decoration-none text-muted" style="font-size: 13px;">&larr; Kembali ke beranda</a>
                    </div>
                </div>
            </div>
        </div>
